...
import { EntitlementEntry, ProcessorStatus, MerkleTreeData } from "./types";
...
export class TreeProcessor {
  ...
  async run(): Promise<void> {
    try {
      ...
      const entitlementsData = await this.storage.retrieveData(
        `user-entitlement-file/${this.projectId}.csv`
      );
      if (!entitlementsData) {
        throw new Error(
          `Entitlements file not found for project ${this.projectId}`
        );
      }
      ...
      // Build Merkle tree
      const values: Array<MerkleTreeData> = entitlements.map((e) => ({
        account: e.address,
        lockup: e.lockup,
        amount: e.amount.includes(".") ? e.amount.split(".")[0] : e.amount,
      }));
      this.merkleTree = await NearMerkleTree.of(values, [
        "account",
        "lockup",
        "amount",
      ]);
      ...
      await this.storage.storeJSON(
        `project-tree/${this.projectId}.json`,
        this.merkleTree.dump()
      );
      ...
      await this.storage.storeJSON(
        `events/build-complete/${dateStr}/${this.projectId}.json`,
        buildResult
      );
      await this.storage.storeJSON(
        `project-build-complete/${this.projectId}.json`,
        buildResult
      );
      ...
      await this.database.updateProject(this.projectId, {
        ...
        buildElapsed: this.buildEndTime
          ? (this.buildEndTime.getTime() - this.buildStartTime!.getTime()) /
            1000
          : undefined,
        totalElapsed: this.buildEndTime
          ? (this.buildEndTime.getTime() - this.startTime.getTime()) / 1000
          : undefined,
      ...
      for (const valueEntry of this.merkleTree.values) {
        const { account, lockup, amount } = valueEntry.value;
        const proof = this.merkleTree.getProof(valueEntry.treeIndex);

        const proofData = {
          campaignId: this.projectId,
          proof,
          address: account,
          lockup: lockup,
          amount: amount,
      ...
      await this.storage.storeJSON(
        `events/publishing-complete/${endDateStr}/${this.projectId}.json`,
        finalResult
      );
      await this.storage.storeJSON(
        `project-publishing-complete/${this.projectId}.json`,
        finalResult
      );
    } catch (error) {
      this.status = "error";
  ...
  getResult(): ProcessorStatus {
    ...
    if (
      ["building", "generating", "publishing", "complete"].includes(this.status)
    ) {
    ...
    if (
      ["generating", "publishing", "complete"].includes(this.status) &&
      this.merkleTree
    ) {
      result.root = this.merkleTree.getRoot();
      if (this.buildStartTime && this.buildEndTime) {
        result.buildElapsed =
          (this.buildEndTime.getTime() - this.buildStartTime.getTime()) / 1000;
        result.totalElapsed =
          (this.buildEndTime.getTime() - this.startTime.getTime()) / 1000;
