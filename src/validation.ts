...
export class ValidationUtils {
  ...
  static validateNearAddress(address: string): void {
    ...
    const isNamedAccount = REGEX_PATTERNS.NEAR_ACCOUNT.test(trimmedAddress) || 
    ...
    const isImplicitWithPrefix = trimmedAddress.startsWith('0x') && 
  ...
  static validateAmount(amount: string): string {
    ...
    return trimmedAmount.includes('.') ? trimmedAmount.split('.')[0] : trimmedAmount;
  }
  ...
  static validateEntitlementEntry(
    entry: { address: string; amount: string },
    lineNumber: number
  ...
  static validateTxHash(txHash: string): void {
    if (!txHash || typeof txHash !== 'string') {
      throw new ValidationError('Transaction hash is required and must be a string');
    }
