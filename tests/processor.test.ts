  ...
  proof: {
    ...
    findMany: vi.fn().mockImplementation(({ where }) => {
      const proofs = Array.from(mockProofs.values()).filter((proof: any) => {
        if (where?.projectId && proof.projectId !== where.projectId)
          return false;
        if (where?.claimed !== undefined && proof.claimed !== where.claimed)
          return false;
        return true;
      });
    ...
    groupBy: vi.fn().mockImplementation(({ where, by }) => {
      ...
      proofs.forEach((proof: any) => {
        ...
        groups[key]._sum.amount = (
          BigInt(groups[key]._sum.amount) + BigInt(proof.amount)
        ).toString();
      });
...
describe("NearClaimProcessor", () => {
    ...
    if (fs.existsSync(testDataPath)) {
      try {
        ...
        try {
          ...
          console.warn(
            "Could not fully clean test directory, continuing:",
  ...
  describe("Tree and Proof Retrieval", () => {
    ...
    it("should handle case-insensitive address lookup", async () => {
      ...
      expect(response.body.address).toBe("alice.near");
    });
  ...
  describe("Large Dataset Handling", () => {
    ...
    it("should handle massive datasets (100K entries)", async () => {
      ...
      while (attempts < maxAttempts && !completed) {
        ...
        if (response.body.status === "complete") {
          ...
          throw new Error(
            `Processing failed: ${response.body.message || "Unknown error"}`
          );
        }
    ...
    it("should handle ultra-massive datasets (500K entries)", async () => {
      ...
      const entitlements = Array.from({ length: 500000 }, (_, i) => ({
        ...
        lockup: `${i.toString(16).padStart(8, "0")}.lockup.near`,
        amount: (BigInt(i) * BigInt("1000000000000000000")).toString(),
      }));
      ...
      while (attempts < maxAttempts && !completed) {
        ...
        if (response.body.status !== lastStatus) {
          console.log(
            `Status changed: ${lastStatus} -> ${response.body.status}`
          );
          lastStatus = response.body.status;
        }
        ...
        if (
          ...
          response.body.generated !== lastProgress
        ) {
          const progress = Math.round((response.body.generated / 500000) * 100);
          console.log(
            `Generated ${response.body.generated}/500000 proofs (${progress}%)`
          );
  ...
  describe("Status Progression", () => {
    ...
    it("should progress through expected statuses", async () => {
      ...
      const hasValidStatus = Array.from(observedStatuses).some((status) =>
